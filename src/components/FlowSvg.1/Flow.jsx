...
export default class Flow extends React.Component {
  ...
  onMathRuleDirectSingle(child, x, y, defaultlen, maxwidth) {
    const width = isUndefined(child.props.width, this.state.width);
    const height = isUndefined(child.props.height, this.state.height);
    const tx = x + defaultlen * 6 + maxwidth;
    const ty = y;
    return { x: tx, y: ty, height: ty + height };
  }
...
/////////////////////////////////////
  ...
  nodeChildDirect(node, x, y, direction, func, funcother) {
    ...
    if (isArray(node)) {
      for (let s = 0; s < node.length; s++) {
        if (s === 0) {
          direct = func(node[s], tx, ty, direction, r, defaultlen, width, height);
          tx = direct.x;
          ty = direct.y;
          tempwidth = direct.width;
        } else {
          direct = funcother(node[s], tx, ty, direction, r, defaultlen, width, height);
          tx = direct.x;
          ty = direct.y;
      ...
      direct = func(node, tx, ty, direction, r, defaultlen, width, height);
      tx = direct.x;
      ty = direct.y;
      tempwidth = direct.width;
    }
    return { x: tx, y: ty, width: tempwidth };
  }
  ...
  nodeChildDirectOther(child, x, y, direction, i, len, func, funcOther) {
    ...
    if (isArray(subChildren)) {
      for (let s = 0; s < subChildren.length; s++) {
        if (s === 0) {
          const res = func(child, subChildren[s], x, y, i,
          ...
          const res = funcOther(subChildren[s], tx, ty,
      ...
      const res = func(child, subChildren, x, y, i,
                       direction, len, r, defaultlen, width, height, last);
      tx = res.x;
      ty = res.y;
      gpChild = res.gpChild;
    ...
    return { x: tx, y: ty, gpChild, width: tempwidth };
  }
    ...
    if (direction === 'vertical') {
      if (isArray(children)) {
        ...
        for (let i = 0; i < child.length; i++) {
          if (i === 0) {
            const res = this.onVerticalFirst(child[i], tx, ty, i, i, direction);
            tx = res.x;
            ty = res.y;
            gpChild = res.gpChild;
            maxwidth = res.width;
          } else {
            const res = this.nodeChildDirectOther(child[i], tx, ty, direction,
        ...
        const res = this.onVerticalFirst(children, tx, ty, 1, this.state.last, direction);
        tx = res.x;
        ty = res.y;
      ...
      if (isArray(children)) {
        ...
        for (let i = 0; i < child.length; i++) {
          ...
          if (kind === 'in') {
            innum = innum + 1;
            if (i === len) {
              gpChild = cloneElement(child[i], tx, ty, i, this.state.last, direction);
            } else {
              gpChild = cloneElement(child[i], tx, ty, i, i, direction);
            }
            if (isArray(subChildren)) {
          ...
          if (kind === 'rule') {
            ...
            if (fronum === 1 && innum === 1) {
              ...
              const ru = this.onMathRuleDirect(subChildren, tx, ty, defaultlen, maxwidth);
              tx = ru.x;
              ty = ru.y;
              ruleMaxHight = ru.height;
            }
          ...
          if (kind === 'out') {
            ...
            maxheight = maxheight > ty ? maxheight : ty;
          }
          groupArr.push(gpChild);
        }
        lineTot = this.onHorizontalLine(horiLineTot);
      } else {
        groupArr.push(cloneElement(children, tx, ty, 1, this.state.last, direction));
      }
      totwidth = x + tx + maxwidth;
      totheight = maxheight;
    }
  ...
  render() {
    ...
    return (
      <div>
        <svg width={ re.width } height={ re.height } >
          {re.children}
          {re.line}
        </svg>
      </div>
