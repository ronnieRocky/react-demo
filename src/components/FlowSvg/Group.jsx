...
export default class Group extends React.Component {
  ...
  onFlash(w, h) {
    console.log("onGroupFlash");
    this.props.onFlash(w, h);
    this.setState({
      width: w,
      height: h,
    });
  ...
  onHorizontalDirect(child, i, tx, ty, tblr, locat, direction, onFlash) {
    const init = this.onCommon(child);
    const node = cloneElement(child, tx, ty, i, locat,
                              direction, init.width, init.height, init.fill, init.stroke, onFlash);
                              console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^node");
                              console.log(node);
    ...
    if (isArray(this.props.children)) {
      ...
      for (let i = 0; i < child.length; i++) {
        ...
        if (direction === 'vertical') {
          ...
          const onFlash = () => {};
          res = this.onHorizontalDirect(child[i], i, tx, ty, tblr, temlocat, direction, onFlash);
          tx = res.x;
          ty = res.y;
          finalCon.push(res.elment);
        }
