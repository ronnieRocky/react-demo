...
export default class Main extends React.Component {
  ...
  onDrag(id,mx,my){
    ...
    console.log(mx);
    console.log(my);
    let con = this.state.shapeContainer;
    for(let i=0;i<con.length;i++){
      if(con[i].id===id){
        ...
        const sid = con[i].id;
        let sourceConnect = con[i].connect;
        ...
        for(let j=0; j< link.length ;j++){
          ...
          let tid = link[j].id;
          let tempcon = this.onDirect(self, target, link.length, j, con[i].locale);
          ...
          let targetConnect = link[j].connect;
          targetConnect.filter(e =>e.key===skv.key).forEach(e=>{
            e.value.length=0;
            e.value=[...skv.value];
          });
          link[j].connect = targetConnect;

         sourceConnect.filter(e =>e.key===tkv.key).forEach(e=>{
            e.value.length=0;
            e.value=[...tkv.value];
          });
  ...
  componentWillMount(){
    var ra = new Shape("ra","rect",100,30);
    var rb = new Shape("rb","rect",100,100);
    var rc = new Shape("rc","rect",100,170);
    var rd = new Shape("rd","rect",100,240);
    let con =[];
    let con2 = [];
    ra.locale="start";
    con2.push(ra);
   ...
   onDirect(self, target ,len, bango,locale){
     ...
     let tempcom=[];
     let middle = this.onMiddleMath(sdirect.x, sdirect.y, tdirect.x, tdirect.y, direct);
  ...
  onLineMath(sx,sy,tx,ty,direct,middle,len,bango,locale){
   ...
   onTBLR(x,y,wh){
      ...
      return {top: top, button:button, left: left, right: right};
  }
